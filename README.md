...
- **Inline rating injection** — On a successful lookup the content script injects a `<span data-bgg-rating>` element immediately after the title, displaying the rating in the format `(BGG: 8.0. See more)` where **See more** is a link to the canonical BGG game page (`target="_blank"`, `rel="noopener noreferrer"`), with styling inherited from the title element.
